...
import '../src/i18n';
import {
  CODEX_PRICE_MODELS, getDefaultSyncSettings, syncCodexPrices, syncPrices,
...
const cost = { input: 2, output: 12, cache_read: 0.2, cache_write: 2.5,
  tiers: [{ tier: { type: 'context', size: 272000 }, input: 4, output: 18, cache_read: 0.4, cache_write: 5 }],
};
...
describe('Codex catalog price synchronization', () => {
  test('syncs the entire catalog without usage records and uses only OpenAI rates', async () => {
    ...
    mockPrices({ openai: { models }, reseller: { models: {
    ...
    expect(result.prices['gpt-5.6-terra']).toEqual({
      input: 2, output: 12, cacheRead: 0.2, cacheCreate: 2.5,
  ...
  test('does not invent zero prices or borrow a missing model from a reseller', async () => {
    mockPrices({ openai: { models: { 'gpt-5.5': {}, 'gpt-6-astra': { cost } } },
  ...
  test('retains advanced sync for other models and accepts explicitly free prices', async () => {
    ...
    expect((await syncPrices(['fixture-model'], getDefaultSyncSettings())).prices['fixture-model'].input).toBe(0);
  });
  ...
  test('price controls are available without usage and the header entry is outside charts', async () => {
    const html = renderToStaticMarkup(createElement(PriceSettingsCard, {
  ...
  test('all new price UI labels are translated', () => {
    for (const locale of [en, zh, tw, ru]) {
      for (const key of ['price_entry', 'sync_codex', 'advanced_sync', 'codex_price_hint',
        'codex_sync_success', 'codex_sync_missing'] as const) {
